...
def gpt_with_history(prompt, history, model="gpt-4.1-nano", temperature=0.7, max_tokens=1000, n=1, stop=None) -> list:
    messages = []
    for h in history:
        if 'answer' in h:
...
def gpt(prompt, model="gpt-4.1-nano", temperature=0.7, max_tokens=1000, n=1, stop=None) -> list:
    messages = [{"role": "user", "content": prompt}]
    return chatgpt(messages, model=model, temperature=temperature, max_tokens=max_tokens, n=n, stop=stop)


def chatgpt(messages, model="gpt-4.1-nano", temperature=0.7, max_tokens=1000, n=1, stop=None) -> list:
...
def gpt_usage(backend="gpt-4.1-nano"):
    global completion_tokens, prompt_tokens
    if backend == "gpt-4":
