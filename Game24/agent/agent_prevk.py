...
class TreeOfThoughtAgentPrevK(Agent):
    ...
    def plan(self, env, to_print=True):
        ...
        value_reflects = self.value_reflects
        if self.k_memory is not None:
            if self.k_memory > 0:
                reflects = reflects[-self.k_memory:]
                value_reflects = value_reflects[-self.k_memory:]
            elif self.k_memory == 0:
                reflects = []
                value_reflects = []
        
        print(20*"-")
        print(f"number of reflects: {len(reflects)}")
        print(f"reflects: {reflects}")
        print(20*"-")
        obs = [{"feedback":prompt},
